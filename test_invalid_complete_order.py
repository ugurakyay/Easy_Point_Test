...
def test_complete_order_with_invalid_otp():
    response = requests.post(f"{BASE_URL}/post/complete", json={
...
def test_complete_order_with_empty_otp():
    response = requests.post(f"{BASE_URL}/post/complete", json={
        "postID": VALID_POST_ID,
        "otpType": "EasypointOTP",
        "otp": ""
...
def test_complete_order_with_invalid_post_id():
    response = requests.post(f"{BASE_URL}/post/complete", json={
